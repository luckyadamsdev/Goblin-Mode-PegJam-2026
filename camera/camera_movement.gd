...
func _physics_process(_delta : float):
	var delta_v := target_position - follow_this.global_position
	delta_v.y = 0.0
	...
	if (delta_v.length() > follow_distance_with_speed):
		...
		last_target_position = target_position
		target_position = follow_this.global_position + delta_v
	...
	var look_position:Vector3 = follow_this.global_position
